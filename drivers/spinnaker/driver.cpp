...
SpinnakerDriver::SpinnakerDriver() : is_capturing(false) {
  ...
  this->color_space_map.insert(ColorSpaceBimap::value_type(ColorSpaces::RGB, "BGR8"));
}

std::vector<CameraInfo> SpinnakerDriver::find_cameras() {
  auto cam_system = spn::System::GetInstance();
  ...
  for (auto i = 0; i < n_cameras; ++i) {
    ...
    int64_t int_value = 0;
    if (get_op_int(cam->GetTLDeviceNodeMap(), "GevDeviceIPAddress", &int_value).code() != StatusCode::OK)
      continue;
    eth->set_ip_address(make_ip_address(int_value));
    if (get_op_int(cam->GetTLDeviceNodeMap(), "GevDeviceSubnetMask", &int_value).code() != StatusCode::OK)
      continue;
    eth->set_subnet_mask(make_subnet_mask(int_value));
    if (get_op_int(cam->GetTLDeviceNodeMap(), "GevDeviceMACAddress", &int_value).code() != StatusCode::OK)
      continue;
    eth->set_mac_address(make_mac_address(int_value));
    if (get_op_int(cam->GetTLDeviceNodeMap(), "DeviceLinkSpeed", &int_value).code() != StatusCode::OK)
      continue;
    info.set_link_speed(int_value);

    std::string str_value = "";
    if (get_op_str(cam->GetTLDeviceNodeMap(), "DeviceModelName", &str_value).code() != StatusCode::OK)
      continue;
    info.set_model_name(str_value);
    if (get_op_str(cam->GetTLDeviceNodeMap(), "DeviceSerialNumber", &str_value).code() != StatusCode::OK)
      continue;
    info.set_serial_number(str_value);
    cam_infos.push_back(info);
  }
  return cam_infos;
}

void SpinnakerDriver::connect(CameraInfo const& cam_info) {
  try {
...
void SpinnakerDriver::start_capture() {
  try {
...
Status SpinnakerDriver::set_image_format(ImageFormat const& imgf) {
...
Status SpinnakerDriver::get_image_format(ImageFormat* imgf) {
  *imgf = this->image_format;
  return is::make_status(StatusCode::OK);
}

Status SpinnakerDriver::set_sampling_rate(pb::FloatValue const& rate) {
...
Status SpinnakerDriver::get_sampling_rate(pb::FloatValue* rate) {
  float value = 0.0f;
...
Status SpinnakerDriver::get_color_space(ColorSpace* color_space) {
  std::string cs_cam;
...
Status SpinnakerDriver::set_resolution(Resolution const& resolution) {
  ...
  auto function = [&](Resolution const& res) -> Status {
    ...
    if (!(width % this->step_h == 0 && height % this->step_v == 0 && equal_div)) {
      auto why = fmt::format("{}x{} isn't a valid resolution. Choose betewen:{}", width, height, this->resolution_info);
      return internal_error(StatusCode::OUT_OF_RANGE, why);
    }
...
Status SpinnakerDriver::get_resolution(Resolution* resolution) {
...
Status SpinnakerDriver::set_region_of_interest(BoundingPoly const& roi) {
  ...
  if (n_verticies > 2)
    return internal_error(StatusCode::UNIMPLEMENTED, "Funtionality implemented just for BoundingPoly with 2 vertices");
...
Status SpinnakerDriver::get_region_of_interest(BoundingPoly* roi) {
...
Status SpinnakerDriver::set_delay(pb::FloatValue const& delay) {
  /*
...
Status SpinnakerDriver::get_delay(pb::FloatValue* delay) {
  delay->set_value(0.0f);
  // return nodes.float_op().get("TriggerDelay") * 1e-6f;
  return is::make_status(StatusCode::OK);
}

Status SpinnakerDriver::set_shutter(CameraSetting const& shutter) {
...
Status SpinnakerDriver::get_shutter(CameraSetting* shutter) {
...
Status SpinnakerDriver::set_gain(CameraSetting const& gain) {
  if (gain.automatic()) {
...
Status SpinnakerDriver::get_gain(CameraSetting* gain) {
  std::string automatic_str;
...
Status SpinnakerDriver::set_brightness(CameraSetting const& brightness) {
  OpRange<float> range;
...
Status SpinnakerDriver::get_brightness(CameraSetting* brightness) {
  float value = 0.0f;
...
Status SpinnakerDriver::set_white_balance(CameraSetting const& wb, std::string const& type) {
  ColorSpace color_space;
...
Status SpinnakerDriver::get_white_balance(CameraSetting* wb, std::string const& type) {
  ColorSpace color_space;
  is_assert_ok(this->get_color_space(&color_space));
  if (color_space.value() != ColorSpaces::RGB)
    return internal_error(StatusCode::INTERNAL_ERROR, "White Balace availabe just on \'RGB\' color space");
...
Status SpinnakerDriver::set_white_balance_bu(CameraSetting const& wb) {
  return this->set_white_balance(wb, "Blue");
}

Status SpinnakerDriver::get_white_balance_bu(CameraSetting* wb) {
  return this->get_white_balance(wb, "Blue");
}

Status SpinnakerDriver::set_white_balance_rv(CameraSetting const& wb) {
  return this->set_white_balance(wb, "Red");
}

Status SpinnakerDriver::get_white_balance_rv(CameraSetting* wb) {
...
Status SpinnakerDriver::get_gamma(CameraSetting* gamma) {
  float value;
...
Status SpinnakerDriver::set_exposure(CameraSetting const& exposure) {
  OpRange<float> range;
...
Status SpinnakerDriver::get_exposure(CameraSetting* exposure) {
  float value;
...
Status SpinnakerDriver::set_hue(CameraSetting const&) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'HUE\' property not implemented for this camera.");
}

Status SpinnakerDriver::get_hue(CameraSetting*) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'HUE\' property not implemented for this camera.");
}

Status SpinnakerDriver::set_saturation(CameraSetting const&) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Saturation\' property not implemented for this camera.");
}

Status SpinnakerDriver::get_saturation(CameraSetting*) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Saturation\' property not implemented for this camera.");
}

Status SpinnakerDriver::set_focus(CameraSetting const&) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Focus\' property not implemented for this camera.");
}

Status SpinnakerDriver::get_focus(CameraSetting*) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Focus\' property not implemented for this camera.");
}

Status SpinnakerDriver::set_zoom(CameraSetting const&) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Zoom\' property not implemented for this camera.");
}

Status SpinnakerDriver::get_zoom(CameraSetting*) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Zoom\' property not implemented for this camera.");
}

Status SpinnakerDriver::set_iris(CameraSetting const&) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Iris\' property not implemented for this camera.");
}

Status SpinnakerDriver::get_iris(CameraSetting*) {
  return internal_error(StatusCode::UNIMPLEMENTED, "\'Iris\' property not implemented for this camera.");
}

Status SpinnakerDriver::set_packet_delay(int const& packet_delay) {
...
Status SpinnakerDriver::set_packet_size(int const& packet_size) {
...
Status SpinnakerDriver::reverse_x(bool enable) {
...
Status SpinnakerDriver::reverse_y(bool enable) {
  auto function = [&](bool e) -> Status {
...
spn::INodeMap& SpinnakerDriver::node_map() const {
  return this->cam->GetNodeMap();
}

std::vector<int> SpinnakerDriver::get_compression_parm() {
  std::vector<int> parm;
